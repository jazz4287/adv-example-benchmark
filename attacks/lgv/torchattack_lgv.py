...
import robustbench.model_zoo.models
import torch
    ...
    def collect_models(self):
        ...
        for i_sample in tqdm(
            ...
            if self.use_cpu_offload:
                model_sample = copy.deepcopy(self.model.to(torch.device("cpu")))
                self.model.to(self.device)
            else:
                model_sample = copy.deepcopy(self.model).to(torch.device("cpu"))
    ...
    def forward(self, images, labels):
        ...
        if not self.base_attack:
            ...
            if not self.use_cpu_offload:
                ...
                self.list_models = [model.to(torch.device("cpu")) for model in self.list_models]

            f_model = LightEnsemble(
                self.list_models, order=self.order, n_grad=self.n_grad, use_cpu_offload=self.use_cpu_offload
            )

            if self._model_training:
                f_model.eval()
            self.base_attack = self.attack_class(
                model=f_model, **self.kwargs_att
            )
        ...
        self.base_attack.device = device
        adv_images = self.base_attack(images, labels)
        return adv_images


class LightEnsemble(nn.Module):
    def __init__(self, list_models, order="shuffle", n_grad=1, use_cpu_offload: bool = False):
        """
    ...
    def forward(self, x):
        ...
        if self.n_grad == 1:
            if self.use_cpu_offload:
                self.models = self.models.to(torch.device("cpu"))
                self.models[indexes[0]] = self.models[indexes[0]].to(self.device)
