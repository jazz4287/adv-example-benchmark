...
from attacks.acg.utils.criterion import get_criterion
from attacks.acg.utils.compute_diversity_index import DI
...
class RestartAttacker(WhiteBoxAttacker):
    ...
    def _attack(self, x, y, *args, **kwargs):
        ...
        bs = x.shape[0]
        # acc
        logits = self.model(adv_example.to(self.device))
        if self.experiment:
        ...
        for start in range(self.num_restarts):
            ...
            single_adv_example = single_adv_example.to(self.device)
            # acc
            logits = self.model(single_adv_example)
        ...
        assert (adv_example >= torch.clamp(x - self.epsilon, min=0)).all().cpu().item()
        assert (adv_example <= torch.clamp(x + self.epsilon, max=1)).all().cpu().item()
