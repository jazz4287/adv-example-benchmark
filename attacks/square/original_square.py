...
class Square(Attack):
        ...
        with torch.no_grad():
            ...
            if self.norm == "Linf":
                ...
                for i_iter in range(self.n_queries):
                    ...
                    ind_succ_numel = ind_succ.numel()
                    if prev_ind_succ is None:
                        prev_ind_succ = ind_succ_numel
                        display = True
                    else:
                        if prev_ind_succ != ind_succ_numel:
                            display = True
                            prev_ind_succ = ind_succ_numel
                        else:
                            display = False
                    if self.verbose and display and ind_succ_numel != 0:
                        print(
                            "{}".format(i_iter + 1),
    ...
    def perturb(self, x, y=None):
        ...
        for counter in range(self.n_restarts):
            ...
            if self.early_stop is not None:
                if len(moving_window) < self.early_stop:
                    moving_window.append(ind_to_fool.numel())
                else:
                    last_perf = moving_window.pop(0)
                    if ind_to_fool.numel() == last_perf:
                        if self.verbose:
                            print(f"No progress has been made in {self.early_stop} restarts. Terminating early.")
                        break
                    else:
                        moving_window.append(ind_to_fool.numel())
